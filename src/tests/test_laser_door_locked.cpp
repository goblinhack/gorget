...
[[nodiscard]] static auto test_laser_door_locked(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
...
[[nodiscard]] auto test_load_laser_door_locked() ->  bool // NOLINT
{
  TRACE();

  Testp test = test_load("laser_door_locked");

  // begin sort marker1 {
  test_callback_set(test, test_laser_door_locked);
  // end sort marker1 }
