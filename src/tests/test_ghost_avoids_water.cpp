...
[[nodiscard]] static auto test_ghost_avoids_water(Gamep g, Testp t) -> bool
{
  TEST_LOG(t, "begin");
  TRACE();
...
auto test_load_ghost_avoids_water() -> bool // NOLINT
{
  ...
  Testp test = test_load("ghost_avoids_water");

  // begin sort marker1 {
  test_callback_set(test, test_ghost_avoids_water);
  // end sort marker1 }
