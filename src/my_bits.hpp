...
#define BIT_SET(bit, _array_) ((_array_)[ (bit) / BITS_PER_SLOT(_array_) ] |= ((unsigned) 1 << ((bit) % BITS_PER_SLOT(_array_))))

#define BIT_CLR(bit, _array_) ((_array_)[ (bit) / BITS_PER_SLOT(_array_) ] &= ~((unsigned) 1 << ((bit) % BITS_PER_SLOT(_array_))))

#define BIT_ISSET(bit, _array_) ((_array_)[ (bit) / BITS_PER_SLOT(_array_) ] & ((unsigned) 1 << ((bit) % BITS_PER_SLOT(_array_))))
...
#define FIRST_BIT_IN_8BIT(_bitarr_)                                                                                                        \
  (((_bitarr_) & 0xFF) ? (((_bitarr_) & 0x0F) ? FIRST_BIT_IN_4BIT(_bitarr_) : FIRST_BIT_IN_4BIT((_bitarr_) >> 4) + 4) : 8)

#define FIRST_BIT_IN_16BIT(_bitarr_)                                                                                                       \
  (((_bitarr_) & 0xFFFF) ? (((_bitarr_) & 0x00FF) ? FIRST_BIT_IN_8BIT(_bitarr_) : FIRST_BIT_IN_8BIT((_bitarr_) >> 8) + 8) : 16)

#define FIRST_BIT_IN_32BIT(_bitarr_)                                                                                                       \
  (((_bitarr_) & 0xFFFFFFFF) ? (((_bitarr_) & 0x0000FFFF) ? FIRST_BIT_IN_16BIT(_bitarr_) : FIRST_BIT_IN_16BIT((_bitarr_) >> 16) + 16) : 32)

#define FIRST_BIT_IN_64BIT(_bitarr_)                                                                                                       \
  ((_bitarr_) ? (((_bitarr_) & 0xFFFFFFFF) ? FIRST_BIT_IN_32BIT(_bitarr_) : FIRST_BIT_IN_32BIT((_bitarr_) >> 32) + 32) : 64)
...
static inline auto bitcount(int w) -> int
{
  w = (0x55555555LU & w) + (0x55555555LU & (w >> 1));
  w = (0x33333333LU & w) + (0x33333333LU & (w >> 2));
  w = (0x0f0f0f0fLU & w) + (0x0f0f0f0fLU & (w >> 4));
