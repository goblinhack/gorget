...
void thing_interpolate(Gamep g, Levelsp v, Levelp l, Thingp t, float dt)
{
  ...
  if (compiler_unused) {
    THING_DBG(g, v, l, t, "interpolate from (%d,%d) to (%d,%d) dt %f", moving_from.x, moving_from.y, at.x, at.y, dt);
  }
  ...
  if (compiler_unused) {
    THING_DBG(g, v, l, t, "from (%d,%d) to (%d,%d) dt %f pix_at (%d,%d)", moving_from.x, moving_from.y, at.x, at.y, dt, pix_at.x, pix_at.y);
  }
}
