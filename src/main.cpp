...
int main(int argc, char *argv[])
{
  ...
  if (! g_opt_tests) {
    TRACE_NO_INDENT();
    if (static_cast<unsigned int>(sdl_init()) == 0U) {
      ERR("SDL: Init");
    }
  }
  ...
  if (! g_opt_tests) {
    TRACE_NO_INDENT();
    if (static_cast<unsigned int>(sdl_display_init(g)) == 0U) {
  ...
  if (! g_skip_audio_and_gfx) {
    ...
    if (static_cast<unsigned int>(font_init()) == 0U) {
      ERR("Font init");
    }
  }
  ...
  if (! g_skip_audio_and_gfx) {
    ...
    if (static_cast<unsigned int>(tex_init()) == 0U) {
      ERR("Tex init");
    }
    wid_console_flush(g);
  }
...

  {
    TRACE_NO_INDENT();
    LOG("Load commands");
    if (static_cast<unsigned int>(command_init()) == 0U) {
      ERR("Command init");
    }
    LOG("Loaded commands");
    wid_console_flush(g);
